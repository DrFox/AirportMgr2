<#
.SYNOPSIS
    Runs the RoadNet automation tests headless and exits non-zero if any failed.
...
.EXAMPLE
    ./Tools/Run-RoadNetTests.ps1
    ./Tools/Run-RoadNetTests.ps1 -Filter RoadNet.Solve
#>
[CmdletBinding()]
param(
    [string] $Filter  = 'RoadNet',
...
$logPath    = Join-Path $projectDir 'Saved\Logs\RoadNetTests.log'
...
Write-Host ''
Write-Host "$($completed.Count) test(s) run, $($failed.Count) failed."
