"""Creates ABP_Plane13, parented to UAirsideAgentAnim and targeting SK_Plane13's skeleton, and
...
THIS CREATES THE ASSET AND NOT ITS ANIMGRAPH. Tools/wire_plane13_anim.py authors the whole
...
want the editor in opposite states, which is one restart:
  ...
  2. editor UP:     Tools/wire_plane13_anim.py, to wire it

THE BONE PLAN AND THE AXIS RESOLVER LIVE IN Tools/Python/airside_anim.py. What is left here
is what is TRUE OF plane13 AND OF NOTHING ELSE: its paths, its bones, and the notes below.

FIFTEEN DRIVEN JOINTS OF SIXTEEN - plane11's set with one axle fewer a side: two TWO-axle
bogies (wheel_L1..L2 / R1..R2, children of gear_L / gear_R), four bay doors, two fans. The
leg doors (legdoor_L/_R) are MESHES skinned to gear_L/_R and ride the leg, as plane11's do.

NO TRUCK BONE, plane11's case: plane13/SPEC.md's rig lists none, the bogie stows untilted, so
TruckTiltSeconds stays zero and nothing here matches BONE_RULES' truck/bogie needles.

THE NOSE LEG FOLDS FORWARD, as the real 757's does and as plane6's does. gear_nose points -X in
Blender, plane6's direction (plane11's points +X and folds aft), so the same positive key folds
it forward and the graph needs no different row.
"""
import os
import sys
...
KEY = "plane13"
SOURCE = r"C:\repos\AirportMgr2Models\plane13\export\plane13.glb"

SKELETON = "/Game/Aircraft/Plane13/SK_Plane13_Skeleton"
MESH = "/Game/Aircraft/Plane13/SK_Plane13"
ABP_PATH = "/Game/Aircraft/Plane13"
ABP_NAME = "ABP_Plane13"

# THE BONES THIS GRAPH DRIVES, and every one turns about ITS OWN LENGTH - plane13's
# build_rig.py keys its cycle about pose-bone Y, as plane11's does. The mirroring lives in the
# rest direction (gear_L and gear_R point opposite ways, so one figure folds both inboard); the
# handedness flip lives in wire_plane13_anim.py's multiplier column. All four wheel bones point
# the same way so one WheelAngleDegrees spins them together.
DRIVEN_BONES = [
    "prop_L", "prop_R",
    "wheel_L1", "wheel_L2",
    "wheel_R1", "wheel_R2",
...
# NO BONE IS DELIBERATELY RAKED, plane11's case. rig_map.json puts nosewheel_steer's head
# (axle) and tail (trunnion) at stations 5.8928 and 5.8293 over 2.51 m of height - 1.45 degrees
# off vertical - and the nose doors' hinge falls 1.7 degrees over its 2.2 m. Both are inside
# resolve_axis's square band (0.99, about 8 degrees). resolve_axis fails a declared-raked bone
# that turns out square, so this list stays honest.
RAKED = ()
...
def report_plan():
    ...
    say("")
    say("THE GRAPH THIS ASSET WANTS. Tools/wire_plane13_anim.py authors it against the")
    ...
    say("")
    say("  THE ROTATION AXIS PER BONE, resolved off SK_Plane13's reference pose in UE's own")
...
def verify(asset, skeleton):
    ...
    if got != skeleton:
        ...
        say("PASS %s targets SK_Plane13_Skeleton" % ABP_NAME)
...
def run():
    ...
    if existing is not None:
        # LOAD, never replace: DA_Aircraft_Plane13 points at this asset's generated class,
        # and Tools/wire_plane13_anim.py rebuilds the GRAPH inside it without replacing it.
