"""Creates ABP_Plane9, parented to UAirsideAgentAnim and targeting SK_Plane9's skeleton, and
...
THIS CREATES THE ASSET AND NOT ITS ANIMGRAPH. Tools/wire_plane9_anim.py authors the whole
...
want the editor in opposite states, which is one restart:
  ...
  2. editor UP:     Tools/wire_plane9_anim.py, to wire it

THE BONE PLAN AND THE AXIS RESOLVER LIVE IN Tools/Python/airside_anim.py. What is left here
is what is TRUE OF plane9 AND OF NOTHING ELSE: its paths, its bones, and the notes below.

THIRTEEN DRIVEN JOINTS OF FOURTEEN - plane5's exact set, bone for bone. What the A320 adds
over plane4's 737 is MAIN-GEAR DOORS: the 737's mains sit in an open well behind a fairing,
the A320's close behind door_main_L/_R. The leg doors (legdoor_L/_R) are MESHES skinned to
gear_L/_R and ride the leg, so they need no bone of their own.
"""
import os
import sys
...
SKELETON = "/Game/Aircraft/Plane9/SK_Plane9_Skeleton"
MESH = "/Game/Aircraft/Plane9/SK_Plane9"
ABP_PATH = "/Game/Aircraft/Plane9"
ABP_NAME = "ABP_Plane9"

# THE BONES THIS GRAPH DRIVES, and every one turns about ITS OWN LENGTH - plane9's
# build_rig.py keys its cycle about pose-bone Y, as plane5's and plane8's do. The mirroring
# lives in the rest direction (gear_L and gear_R point opposite ways, so one figure folds both
# inboard); the handedness flip lives in wire_plane9_anim.py's multiplier column.
DRIVEN_BONES = [
    "prop_L", "prop_R",
...
# THE ONE BONE THAT IS DELIBERATELY NOT SQUARE TO THE AIRFRAME, for plane6's reason: a nose
# leg steers about its STRUT, and this one is raked - rig_map.json puts nosewheel_steer's head
# on the axle at station 5.084 and its tail on the trunnion at 5.367, 1.9 m higher, which is
# 8.5 degrees off vertical.
#
# NOT the nose doors, although plane6's and plane8's are named: build_bays.py's fitted hinge
# here is (0, 0.998, -0.056), 3.2 degrees off level and inside resolve_axis's square band.
# A stale declaration fails, so this list is checked both ways every run.
...
def report_plan():
    ...
    say("")
    say("THE GRAPH THIS ASSET WANTS. Tools/wire_plane9_anim.py authors it against the")
    ...
    say("")
    say("  THE ROTATION AXIS PER BONE, resolved off SK_Plane9's reference pose in UE's own")
...
def verify(asset, skeleton):
    ...
    if got != skeleton:
        ...
        say("PASS %s targets SK_Plane9_Skeleton" % ABP_NAME)
...
def run():
    ...
    if existing is not None:
        # LOAD, never replace: DA_Aircraft_Plane9 points at this asset's generated class,
        # and Tools/wire_plane9_anim.py rebuilds the GRAPH inside it without replacing it.
