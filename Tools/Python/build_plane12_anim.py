"""Creates ABP_Plane12, parented to UAirsideAgentAnim and targeting SK_Plane12's skeleton, and
...
THIS CREATES THE ASSET AND NOT ITS ANIMGRAPH. Tools/wire_plane12_anim.py authors the whole
...
want the editor in opposite states, which is one restart:
  ...
  2. editor UP:     Tools/wire_plane12_anim.py, to wire it

THE BONE PLAN AND THE AXIS RESOLVER LIVE IN Tools/Python/airside_anim.py. What is left here
is what is TRUE OF plane12 AND OF NOTHING ELSE: its paths, its bones, and the notes below.

FIVE DRIVEN JOINTS OF SIX - plane10's set, bone for bone: one prop, two mains, and the nose
pair. FIXED GEAR, so no gear or door bone exists to drive. The spinner is skinned to `prop`
and turns with it; `nosegear` (the fork) is skinned to nosewheel_steer.

THE AEROPLANE SITS 5.02 DEGREES NOSE-UP ON ITS GEAR (plane12/README.md), so the prop's thrust
line and the steer bone's oleo are pitched with it. resolve_axis calls a bone square inside
about 8 degrees (0.99), so 5 degrees of pitch alone raises nothing; RAKED below is what the
measurement said: the nose oleo.
"""
import os
import sys
...
SKELETON = "/Game/Aircraft/Plane12/SK_Plane12_Skeleton"
MESH = "/Game/Aircraft/Plane12/SK_Plane12"
ABP_PATH = "/Game/Aircraft/Plane12"
ABP_NAME = "ABP_Plane12"

# THE BONES THIS GRAPH DRIVES, and every one turns about ITS OWN LENGTH - plane12's
# build_rig.py points each bone down the axis it spins or steers about (wheels along the axle,
# prop along the pitched thrust line, steer up the oleo), as plane10's rig does. The
# handedness flip lives in wire_plane12_anim.py's multiplier column.
DRIVEN_BONES = [
    "prop",
    "wheel_L", "wheel_R", "nosewheel", "nosewheel_steer",
]

# THE NOSE OLEO, MEASURED 8.7 DEG OFF VERTICAL in UE on 2026-09-25 - its drawn rake plus the
# aeroplane's 5.02 deg static pitch - just past resolve_axis's ~8 deg square band. plane6's
# ruling: a nose leg steers about its STRUT, so the rake is right, not a rigging fault. The prop
# is pitched the same 5.02 and stays inside the band. A stale declaration fails both ways.
...
def report_plan():
    ...
    say("")
    say("THE GRAPH THIS ASSET WANTS. Tools/wire_plane12_anim.py authors it against the")
    ...
    say("")
    say("  THE ROTATION AXIS PER BONE, resolved off SK_Plane12's reference pose in UE's own")
...
def verify(asset, skeleton):
    ...
    if got != skeleton:
        ...
        say("PASS %s targets SK_Plane12_Skeleton" % ABP_NAME)
...
def run():
    ...
    if existing is not None:
        # LOAD, never replace: DA_Aircraft_Plane12 points at this asset's generated class,
        # and Tools/wire_plane12_anim.py rebuilds the GRAPH inside it without replacing it.
