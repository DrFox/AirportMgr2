"""Creates ABP_Plane11, parented to UAirsideAgentAnim and targeting SK_Plane11's skeleton, and
...
THIS CREATES THE ASSET AND NOT ITS ANIMGRAPH. Tools/wire_plane11_anim.py authors the whole
...
want the editor in opposite states, which is one restart:
  ...
  2. editor UP:     Tools/wire_plane11_anim.py, to wire it

THE BONE PLAN AND THE AXIS RESOLVER LIVE IN Tools/Python/airside_anim.py. What is left here
is what is TRUE OF plane11 AND OF NOTHING ELSE: its paths, its bones, and the notes below.

SEVENTEEN DRIVEN JOINTS OF EIGHTEEN - plane6's exact set, bone for bone: two three-axle
bogies (wheel_L1..L3 / R1..R3, children of gear_L / gear_R), four bay doors, two fans. The
leg doors (legdoor_L/_R) are MESHES skinned to gear_L/_R and ride the leg, as plane9's do.

NO TRUCK BONE, plane6's case and not plane8's: plane11/SPEC.md says the bogie stows untilted,
so TruckTiltSeconds stays zero and nothing here matches BONE_RULES' truck/bogie needles.

THE NOSE LEG FOLDS AFT, where the real A350's goes forward - a modelling departure ruled in
plane11/SPEC.md. gear_nose points +X in Blender (plane6's points -X), so the same positive key
folds it the other way and the graph needs no different row.
"""
import os
import sys
...
KEY = "plane11"
SOURCE = r"C:\repos\AirportMgr2Models\plane11\export\plane11.glb"

SKELETON = "/Game/Aircraft/Plane11/SK_Plane11_Skeleton"
MESH = "/Game/Aircraft/Plane11/SK_Plane11"
ABP_PATH = "/Game/Aircraft/Plane11"
ABP_NAME = "ABP_Plane11"

# THE BONES THIS GRAPH DRIVES, and every one turns about ITS OWN LENGTH - plane11's
# build_rig.py keys its cycle about pose-bone Y, as plane6's and plane9's do. The mirroring
# lives in the rest direction (gear_L and gear_R point opposite ways, so one figure folds both
# inboard); the handedness flip lives in wire_plane11_anim.py's multiplier column. All six
# wheel bones point the same way so one WheelAngleDegrees spins them together.
DRIVEN_BONES = [
    "prop_L", "prop_R",
...
# VERTICAL leg, because this one folds aft rather than having to lean past a nose cone. The
# nose doors' hinge falls 1.7 degrees over its 2.2 m, inside resolve_axis's square band.
# resolve_axis fails a declared-raked bone that turns out square, so this list stays honest.
RAKED = ()
...
def report_plan():
    ...
    say("")
    say("THE GRAPH THIS ASSET WANTS. Tools/wire_plane11_anim.py authors it against the")
    ...
    say("")
    say("  THE ROTATION AXIS PER BONE, resolved off SK_Plane11's reference pose in UE's own")
...
def verify(asset, skeleton):
    ...
    if got != skeleton:
        ...
        say("PASS %s targets SK_Plane11_Skeleton" % ABP_NAME)
...
def run():
    ...
    if existing is not None:
        # LOAD, never replace: DA_Aircraft_Plane11 points at this asset's generated class,
        # and Tools/wire_plane11_anim.py rebuilds the GRAPH inside it without replacing it.
