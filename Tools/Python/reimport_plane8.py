"""Re-imports SK_Plane8 IN PLACE after a re-export. Run headless:
...
see its header and the memory import-models-is-first-import-only. SK_Plane8 is referenced by
M_ModelYard, DA_Aircraft_Plane8 and ABP_Plane8's skeleton, so a delete-and-recreate would
...
THE FIRST RE-EXPORT, 2026-09-23, CHANGED SKIN AND GLASS, NOT THE RIG: a livery material
(plane8_livery) joined the six the first import carried, and the windscreen was re-cut. The
bone check below is still run every time, because reimport_plane3.py records what a changed
bone tree does to a headless reimport - the mesh follows, the Skeleton does not - and a
future re-export that re-parents a bone has to fail here rather than half-land.
...
def main():
    ...
    say("plane8: %s" % ("ALL CHECKS PASSED" if ok else "SOME CHECKS FAILED - see above"))
    say("DONE")
