"""Re-imports SK_Plane12 IN PLACE after a re-export. Run headless:
...
see its header and the memory import-models-is-first-import-only. SK_Plane12 is referenced by
M_ModelYard, DA_Aircraft_Plane12 and ABP_Plane12's skeleton, so a delete-and-recreate would
...
THE FIRST RE-EXPORT, 2026-09-25, TOOK A MATERIAL AWAY: the two cowl inlets beside the spinner
were painted plane12_intake, which read as two black blotches on the nose, and were repainted
body in piperPA28.blend. No face uses intake now, so the export drops it and the mesh goes
from eight slots to seven. The bone check still runs every time, for plane8's reason.
...
def main():
    ...
    # AND THE OTHER WAY, because this re-export's point is a slot going AWAY. A reimport that
    # kept plane12_intake would pass the check above and still paint the nose black.
    stale = [m for m in after if m not in wanted]
    if stale:
        fail("the mesh still carries %s, which the export no longer declares" % ", ".join(stale))
        ok = False
    else:
        say("PASS no slot survives that the export dropped")
    ...
    say("plane12: %s" % ("ALL CHECKS PASSED" if ok else "SOME CHECKS FAILED - see above"))
    say("DONE")
