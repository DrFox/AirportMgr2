"""Authors ABP_Plane10's AnimGraph - five bones - and reads it back to prove it.

  python Tools/wire_plane10_anim.py            # wires, compiles, saves, verifies
  python Tools/wire_plane10_anim.py --verify   # verifies only, changes nothing
  python Tools/wire_plane10_anim.py --read     # prints the chain as it stands, no assertions

THE EDITOR MUST BE RUNNING; Tools/Python/build_plane10_anim.py is the half that needs it
closed, and writes the measured rotation axes this one refuses to run without.

THE MECHANISM IS Tools/wire_anim_lib.py. WHAT IS LEFT HERE is plane10's own: its Blueprint and
its five bones in chain order - plane1's set, bone for bone.

FIXED GEAR, SO NO RETRACT ROWS. The Caravan's legs are welded on; its rig has no gear or door
bone, and DA_Aircraft_Plane10 declares no gear cycle. The graph is the prop, the three tyres
and the steer.
...
MODEL = wire_anim_lib.Model("plane10", "/Game/Aircraft/Plane10/ABP_Plane10", PLAN)
