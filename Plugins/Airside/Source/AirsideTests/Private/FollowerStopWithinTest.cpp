...
bool FFollowerStopWithinTest::RunTest(const FString& Parameters)
{
	...
	FChassis Chassis;   // Accel 100, Decel 200, SpeedCap 1000 - the struct defaults
	FRouteFollower Follower;
	Follower.Start(Plan, Chassis);

	// Run up to cruise with nothing ahead.
	FVector2D At; double Heading = 0.0;
	for (int32 Tick = 0; Tick < 400; ++Tick) { Follower.Advance(0.05, Chassis, At, Heading); }
	...
	for (int32 Tick = 0; Tick < 400; ++Tick)
	{
		...
		Follower.Advance(0.05, Chassis, StopWithin, At, Heading);
	...
	Follower.Advance(0.05, Chassis, TNumericLimits<double>::Max(), At, Heading);
