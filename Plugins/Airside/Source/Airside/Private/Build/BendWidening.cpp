...
void BendWidening::Rim(const FJunctionInput& Input, const FJunctionResult& Result, FWidening& InOut,
	TArray<FVector2D>& OutRim, double& OutShortfall)
{
	OutRim.Reset();
	OutShortfall = 0.0;
	FInnerEdge Edge;
	if (!InnerEdgeOf(Input, Result, Edge) || Edge.K != InOut.Corner)
	{
		return;
	}
	// Where the cuts ARE: the envelope leans to nothing at each, whatever it needed.
	const double Start = Edge.AlongA - Result.Arms[Edge.K].CutDistance;
	const double End = Edge.ArcLength() + Result.Arms[Edge.K1].CutDistance - Edge.AlongB;
	for (int32 I = 0; I < InOut.U.Num(); ++I)
	{
		const double Allowed = FMath::Max(0.0, Taper * FMath::Min(InOut.U[I] - Start, End - InOut.U[I]));
		const double Kept = FMath::Min(InOut.W[I], Allowed);
		OutShortfall = FMath::Max(OutShortfall, InOut.W[I] - Kept);
		InOut.W[I] = Kept;
	}

	// The rim: every envelope point, and the fillet's own arc at the solver's density so an
	// unwidened stretch of it stays round - in order along u, between (not on) the two cut vertices
	// SolveBoundary lays either side.
	TArray<double> Us;
	for (int32 I = 0; I < InOut.U.Num(); ++I)
	{
		if (InOut.W[I] > 0.0) { Us.Add(InOut.U[I]); }
	}
	const int32 ArcSteps = FMath::Max(Input.ArcSegments, FMath::CeilToInt32(Edge.ArcLength() / Bin));
	for (int32 Step = 0; Step <= ArcSteps; ++Step)
	{
		Us.Add(Edge.ArcLength() * Step / ArcSteps);
	}
	Us.Sort();
	double Previous = -TNumericLimits<double>::Max();
	for (const double U : Us)
	{
		if (U <= Start + 1.0 || U >= End - 1.0 || U - Previous < 1.0)
		{
			continue;
		}
		OutRim.Add(Edge.At(U, WidthAt(InOut, U)));
		Previous = U;
	}
}
