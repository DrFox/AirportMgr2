	...
	const TArray<FRoadSegmentId>& ArmSegments, int32& OutStripSlot, int32& OutFanSlot) const
{
	...
	for (const FRoadSegmentId ArmSegment : ArmSegments)
	{
		...
		const URoadProfile* ArmProfile = Seg ? Network.ProfileFor(*Seg) : nullptr;
...
{
	// The degenerate/sliver guards and the winding flip now live on the buffers themselves
	// (FRoadMeshBuffers::AppendTriangleUp), shared with MarkingQuads::AddQuad (#103) - see
	// that method's own comment for the WHY that used to be here.
...
void FRoadMeshBuilder::AddSegment(const URoadNetwork& Network, FRoadSegmentId SegmentId, int32 RibbonSegments)
{
	...
	// PROFILE FOR, NOT Segment->Profile. This is the second reader the fallback was
	// introduced to abolish, and it survived the first fix because it fails INVISIBLY: the
	// solver reads ProfileFor, so a reloaded segment is still cut to the right width and the
	// ribbon looks perfect. Only the attributes derived here go wrong.
	//
	// What that cost: with no profile there are no bands, so every lateral below came out
	// zero. M_RoadSurface places the centreline from UV1.X as
	// 1 - saturate((|lateral| - CentrelineWidth) * MarkingSharpness), and a lateral of zero
	// makes that mask 1 across the whole width - so the entire road painted MarkingColor.
	// A correctly shaped, correctly sized road, in the right material slot, solid yellow,
	// and only ever after a reload. It was reported as the road tool changing materials and
	// took six wrong diagnoses, every one of them aimed at the material.
	const URoadProfile* SegProfile = Network.ProfileFor(*Segment);
