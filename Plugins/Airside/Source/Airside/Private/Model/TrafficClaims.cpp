...
double FClaimPass::CentreOf(const FRoadAgent& Agent)
{
	...
	const double Ahead = Agent.Airframe.Chassis.BodyCentreX - Agent.Airframe.Chassis.SteerAxleX;
...
FClaimPass::FClaimWindow FClaimPass::WindowFor(const FRoadAgent& Agent) const
{
	...
	const double Decel = FMath::Max(KINDA_SMALL_NUMBER, Agent.Airframe.Chassis.Ground.Taxi.Decel);
	//
