...
// THE AIRFRAME PHYSICS BUNDLE, split out of Model/RoadEntity.h by #176. Climb, approach,
// engine and gear performance, plus the FChassis that rolls on the ground - everything
// FLandingRun and FTakeoffRun need to fly one aeroplane. The ground half (ground performance,
// wheelbase, steer law) moved to Model/Chassis.h on 2026-09-23 - see that file for why - and
// FRouteFollower, FReverseRun and FSpeedProfile take only that half now. Nothing here is
// about where an entity sits on the apron (that stayed in RoadEntity.h) or what the view
// needs to draw it (that is Model/AgentMotion.h). The three used to be one header, so a
// changed turn rate recompiled every file that only wanted to know what EServiceRole an
// anchor was.
 ...
 * which is what zero means on FChassis::MainGearTrack. The difference is that there is no
...
struct AIRSIDE_API FAirframe
{
	...
	/**
	 * How this aeroplane rolls: ground performance, axles, gear track, body centre, steer law.
	 * See FChassis for why they left this struct - a ground consumer takes Chassis alone.
	 */
