...
namespace RoadHeal
{
	ROADNET_API FRoadDeletionPlan PlanNodeDeletion(const URoadNetwork& Network,
		FRoadNodeId Target, const FRoadPlacementLimits& Limits);

	/** How many roads Neighbour keeps once Target and its segments are gone. */
	ROADNET_API int32 DegreeWithout(const URoadNetwork& Network, FRoadNodeId Neighbour, FRoadNodeId Target);
}
