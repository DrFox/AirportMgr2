...
2. **`FEntityInstance::ResolvedAnchors` is parallel to `Definition->Anchors` by index**, with nothing enforcing it. **Worse than this plan first recorded:** the final review showed the natural pattern — iterate the definition, index the instance — reads OUT OF BOUNDS the moment a saved asset gains an anchor, which is ordinary designer work rather than an edge case. Verified by removing the guard: it crashes, `Array index out of bounds: 2 into an array of size 2`. `GetAnchorNode` now bounds-checks the read, but **that guards the symptom, not the source**: an instance placed before its definition gained an anchor still has no node for the new role, which silently has nowhere to go. The real fix — `ResolvedAnchors` as `TArray<FResolvedAnchor>` carrying `{ FGuidelineNodeId Node; EServiceRole Role; }`, retiring the unenforced invariant entirely — plus a load migration or lazy resolution, belongs to the plan that makes definitions real assets.

3. **The two anchor accessors bounds-check different arrays**, deliberately: `GetAnchorWorldHeading` against the definition's `Anchors`, `GetAnchorNode` against the instance's `ResolvedAnchors`. On a definition that gained an anchor they disagree for the new index — one succeeds where the other returns null. Each header documents its own choice; **neither cross-references the other**, so a caller reading one has no signal to check the sibling. Two lines of comment, parked from the final fix wave.

4. **`GetAnchorWorldHeading` returns an unnormalised sum.** 7π/4 + π/2 gives 9π/4, not π/4. Deliberate — every consumer feeds it to trigonometry, where it makes no difference, and wrapping would have weakened the test that pins the composition. A caller comparing two headings for equality must wrap first, and the header does not say so in as many words.

5. **Two of spec §6's nine marking rows now read "not derivable yet"** — the zebra crossing and the stand lead-in line. Recording is not fixing. Both need the same missing thing.
6. **`MakeStandTransient`'s anchor offsets are invented**, not taken from any real stand layout. They are plausible metres-scale values for testing geometry, not an authored stand.
7. **`RemoveEntity` removes its anchor nodes unconditionally**, including any the player has since drawn guidelines to — `RemoveGuidelineNode` takes incident edges with it. Deleting a stand therefore deletes the taxi lines leading into it. Arguably correct, but it is a policy decision made by omission here; the build tool should confirm it.
8. **Nothing tests an apron and a guideline together.** Task 5 places both, but no assertion relates them, because "a guideline lies on a surface" is §7 validation and out of scope.
