...
| K3 | Collinear nodes emit three degenerate triangles of about 1.6e-10 uu². | `Solve/JunctionSolver.cpp` | They pass the CCW gate and are geometrically harmless. `FRoadMeshBuilder::AddTriangle` (Slice 2a) drops only **index**-degenerate triangles, so these near-collinear slivers still pass into `FDynamicMesh3` unchanged. | **Deferred to Slice 2b:** the mesh builder should drop zero-area triangles rather than the solver applying an area heuristic it cannot calibrate. Not done in Slice 2a because the threshold cannot be calibrated yet - this slice has no material or texel scale to judge "too small" against. |
