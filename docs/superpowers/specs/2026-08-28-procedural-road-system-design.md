...
| **2a. Mesh geometry** | network→solver adapter, persisted cut vertices, `FRoadMeshBuilder`, `IRoadMeshSink`, batched `UDynamicMeshComponent`s | the gallery renders as **solid surfaces with no visible crack at any junction** |
| **2b. Materials + markings** | dual UVs, asphalt, shoulder fade, UV1 centreline markings | the gallery matches the reference runway's quality |
...
|---|---|---|---|---|
| K1 | `AddCutVertex`'s weld-**replace** can overwrite one cut vertex with another. At a collinear pass-through node arm 0's `LeftCut` is replaced by arm 1's `RightCut`, so the rim carries 3 of its 4 cut vertices. | `Solve/JunctionSolver.cpp`, `SolveBoundary` | **Stale as of Slice 2a:** "Slice 1 has no consumer that searches the rim" no longer holds - `FRoadMeshBuilder` is such a consumer. It still waits because that consumer welds by exact value rather than searching, so K1 re-shapes into a different failure (see next column) instead of the one originally predicted, and today's gallery has no collinear pass-through node to trigger it. Section 11's R9 auto-subdivide makes collinear interior nodes the **common** case in Slice 3, at which point K1 stops being an edge case. | **Corrected:** not the search failure originally predicted here. `FRoadMeshBuilder` welds rather than searches, so the overwritten rim slot instead produces a duplicate coincident vertex: at a collinear pass-through node the rim carries arm 1's `RightCut` in the slot where arm 0's `LeftCut` was overwritten, so `WeldVertex(arm0.LeftCut)` misses and appends a second vertex about 1e-13 uu away. Invisible, and unreachable in today's gallery. |
| K2 | ~~The model persists only the scalar `TrimA`/`TrimB`.~~ **RESOLVED in Slice 2a:** the solver now writes `LeftCutA/RightCutA/LeftCutB/RightCutB` into `FRoadSegment` alongside the trims, and the mesh builder is forbidden from recomputing them. | `Model/RoadNode.h` | — | Was the load-bearing carried issue. Making the contract structural rather than conventional is the whole point: a convention is exactly what failed in the original Blueprint system. |
| K3 | ~~Collinear nodes emit three degenerate triangles of about 1.6e-10 uu².~~ **RESOLVED in Slice 2b-ii:** `FRoadMeshBuilder::AddTriangle` drops any triangle below `MinTriangleArea`, in addition to the index-degenerate ones it already dropped. | `Build/RoadMeshBuilder.cpp` | — | Resolved in the mesh builder, as this row proposed, rather than by the solver applying an area heuristic. **The threshold is absolute, not texel-derived:** 1e-6 uu² is a square 0.001 uu on a side, below anything a pixel can cover at any plausible texel density, so it needs no material scale to justify - which is also why waiting for one was not in fact necessary. **The description above was wrong about the trigger:** an *exactly* collinear node emits no fan at all (no apex sees its rim, so `SolveBoundary` declines), and the slivers come from a node a hair *off* collinear. Measured on a 3-node line with the middle node offset by 1e-6 uu: 9 triangles, 6 of them slivers, smallest 2.6e-07 uu². Covered by `RoadNet.Build.BandWeld`. |
