...
Seven, in priority order - most specific to what the player is doing, first. Every one is
...
| 2 | Aligned | a placed entity's pose direction and perpendicular | yes |
| 3 | Collinear | the line an existing segment already lies on | yes |
| 4 | Parallel | the nearest road's direction, and its perpendicular | yes |
| 5 | Runway | every runway's heading, and its perpendicular | yes |
| 6 | World | 0/45/90/135 degrees | no |
| 7 | Offset | **distance**: the gap a neighbouring parallel road already keeps | yes |
...
1. **Eligibility.** A candidate is eligible when the cursor's own direction is within
   `ToleranceDegrees` of it. Default 7 degrees.
2. **Ranking.** Among eligible candidates, the smallest angular error wins; ties break on the
   source order in §3.
3. **Hysteresis.** The PREVIOUS frame's winner is kept unless a challenger beats it by
   `StickinessDegrees`. Default 2 degrees. This is what needs state, and why the driver owns
   the chain.
4. **Suspension.** While the suspend key is held, nothing is eligible and `bActive` is false.
...
The tool emits, when `Context.Guide.bActive`:

- `Sink.Line(Point, Winner.ReferenceAt, EPreviewStyle::Guide)` - the dashed line to the thing.
- `Sink.Label(Point, Winner.Description, EPreviewStyle::Guide)` - "parallel to taxiway".
...
1. `ToleranceDegrees` at 7 and `StickinessDegrees` at 2 are placement feel, judged in PIE.
   They start as constants and become `UAirsideSettings` knobs if they need tuning, the same
   route `ClearanceUu` and `GateCorridorUu` took.
