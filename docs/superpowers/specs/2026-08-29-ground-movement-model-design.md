...
4. **Guideline handles do not survive a rebuild.** As implemented, `FRoadGuidelineBuilder::Build` destroys and re-adds every derived node and edge rather than updating them in place, so every generation counter advances on every run. §4.3's entity anchors resolve to guideline nodes and store those handles; unless the builder is changed to update in place, an anchor will dangle the first time anything triggers a rebuild. Decided during Plan A's execution and recorded here rather than fixed, because reusing in place is a substantial rewrite. Whichever way it is resolved belongs to the plan that introduces entities.

5. **Pedestrians are the first flow-type class.** A simulation slice that assumes every class queues like an aircraft will be wrong for them, and wrong in a way that looks like a bug rather than a design gap.
